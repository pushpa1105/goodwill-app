...
export const AchievementForm = ({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      await axios.patch(`/api/webinars/speakers/${speakerId}`, values);
      toast.success("Achievement updated succesfully.");
      toggleEdit();
      router.refresh();
    } catch (error) {
        ...
        <>
          <div className="border rounded shadow-sm bg-violet-100 p-2 mb-2">
            <p
              className={cn(
                "text-sm mt-2",
                !initialData.achievement && "text-slate-500 italic"
              )}
            >
              {!initialData.achievement && "No achievement Info"}
              {initialData.achievement && (
                <Preview value={initialData.achievement} />
              )}
            </p>
          </div>
        </>
