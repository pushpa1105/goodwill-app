...
const formSchema = z.object({
  videoText: z.string().min(1, {
    message: "Field is required",
  }),
});

export const VideoTextForm = ({ initialData, productId }: TitleFormProps) => {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      videoText: initialData.videoText || "",
    },
  });
      ...
      {!isEditing ? (
        <>
          <div className="border rounded shadow-sm bg-violet-100 p-2 mb-2">
            <p className="text-sm mt-2">{initialData.videoText}</p>
          </div>
        </>
      ) : (
        <Form {...form}>
